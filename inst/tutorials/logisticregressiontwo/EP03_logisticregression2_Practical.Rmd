...
The following questions test your knowledge of logistic regression, odds and odds ratios. For some questions it might be convenient to have R or Excel open to use as a calculator. In this set of questions we also focus on model diagnostics. 
...
quiz(
  question("True or false (check all that apply)", 
    type='multiple',         
    answer("In regression an outlier is a value with an unusual value of the dependent variable given the independent variables", correct=TRUE),
    answer("Observations with high leverage have unusual values of the independent variables.", correct=TRUE ),
    ...
    answer("The dfbetas say something about how influential an observation is.", correct=TRUE),
...
In a post-marketing surveillance study, three different antibiotics were compared with respect to adverse reactions in patients with either tonsillitis, sinusitis or a bronchopulmonary infection. Each treatment group contained about 1850 patients. 
...
crosstabulation. Also a logistic regression model was estimated using only the treatment
...
quiz(
  question("Which of the following statements is true? (check all that apply)",
    ...
    answer("If this model is correct,the probability of having an adverse event is highest for people that are approximately 50 years old (if other variables are constant)", correct=TRUE, message="Consider that ax^2+bx+c has its extreme at -b/2a (just take the derivative). So because a and b are in the table of coefficients we can derive the age where risk is highest."),
...
In this question you are being asked to calculate an odds ratio that does not directly correspond to a single coefficient in the table above. Remember that if needed you can always derive the odds ratio as the antilog (exponential) of the difference in log odds (like we did to derive the interpretation of the coefficients in the logistic regression model).
...
```{r Q6}
quiz(
  question("What is the probability of an adverse event of a 50 years old man treated with roxitromycin)",
...
# To describe the relation you can visualize it. For example by a boxplot. The 
...
# We know  how to calculate the log odds  between the levels of 
# LOC. So what would the log odds be from from thoracic to the other levels?
# Knowing the log odds allows us to compute the OR.
...
contrasts(fracture$LOC) <- contr.sum(3)
glm37 <-glm(DEF~LOC+STEN,data=fracture, family = binomial)
glm37
exp(coef(glm37))
# restore contrast
...
```{r P8-hint-2}
# We can make a data set for prediction with
...
```{r P9-hint-1}
# Use the code as in the question.
```
...
```{r P10-hint-1}
# The ROC functions need the predicted values of the model and the true outcomes as inputs. See the course notes for details.
```
